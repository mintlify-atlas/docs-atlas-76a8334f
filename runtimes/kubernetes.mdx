...
description: "Sentinel's KubernetesAgent uses the official Python Kubernetes SDK to investigate pod failures, CrashLoopBackOff, OOMKills, and deployment issues with six read-only tools."
---
...
<Tabs>
  ...
  <Tab title="Scale action">
    In cases where the deployment should be scaled (e.g. scaling down a broken rollout), the action executor also accepts:

    ```bash
    kubectl scale deployment/<name> --replicas=<0-10> -n <namespace>
    ```

    This must be the exact proposed action value — the allowlist enforces `--replicas` between 0 and 10.
  </Tab>
</Tabs>
