...
description: "How Sentinel's human-in-the-loop approval flow works: review the agent's proposed command, then approve, reject, or postpone the remediation action."
---
...
<Tabs>
  ...
  <Tab title="Kubernetes">
    `kubectl` is invoked as a subprocess. If `K8S_PROXY_URL` is configured in the environment, the `--server` flag is injected to route through that URL. `stdout` and `stderr` are captured the same way as Docker actions.
  ...
  <Tab title="PostgreSQL">
    The psycopg2 driver executes the function directly against the target database using the `POSTGRES_DSN` (or individual `POSTGRES_HOST` / `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB` environment variables) connection string from the environment. No shell subprocess is involved.
